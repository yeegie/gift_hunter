...
async def gift_catalog(message: Message, bot: Bot):
    ...
    await message.edit_text(f"<b>🎁 Купить вручную</b>", reply_markup=gift_catalog_keyboard(gifts))
...
async def gift_catalog(callback: CallbackQuery, bot: Bot):
    gifts = await ioc.get(GiftObserver).get_all_gifts()
    await callback.message.edit_text(f"<b>🎁 Купить вручную</b>", reply_markup=gift_catalog_keyboard(gifts))
